...
from Item import *
from Door import Door, LockedDoor, SeceretDoor, SeceretLockedDoor
from Openables import *
from TextStuff import describeAttic, describeBallroom, describeBilliard, describeBushes, describeConservatory, describeDining, describeGarden, describeHall, describeKitchen, describeLibrary, describeLounge, describeStudy, madeItOutNotCurseFree
...
class Lounge(Room):
    def __init__(self, name):
        super().__init__(name)
        self.locations = [HauntedItem("sword", 3), Item("coffee table"), OpenableThing("cabinate", [TakableThing("green key"), HolyItem("silver coin", 1)])]
...
class DiningRoom(Room):
    def __init__(self, name):
        super().__init__(name)
        self.locations = [OpenableThing("Dining Table", [HauntedItem("fruit salad", 2), HauntedItem("glass of water", 2), HauntedItem("fork", 1)]), OpenableThing("Buffet", [TakableThing("yellow key")])]
        ...
        self.locations[1].contents[0].description = ""
...
class Bushes(Room):
    def __init__(self, name):
        super().__init__(name)
        self.locations = [TakableThing("keykey")]
...
class Crypt(Room):
    def __init__(self, name):
        super().__init__(name)
        self.locations = [LockedOpenableThing("coffin", [HolyItem("cross", 5), TakableThing("gold key")], "keykey"), HauntedItem("suit of armour", 6)]
    ...
    def enter(self, player):
        ...
        Door("crypy door", Crypt("the Crypt"), Bushes("the Bushes"), "green key").keyName = "gold key"
